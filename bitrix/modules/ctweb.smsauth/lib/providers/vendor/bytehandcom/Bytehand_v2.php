...
class ByteHandApi {
    ...
    public function send($number, $msg, $msgCharset = 'utf-8') {
        ...
        if (empty($number)) {
            return array("error"=>'Phone number must be set');
        }

        if (empty($msg)) {
            return array("error"=>'Message text must be specified');
        }
