...
class cwsa_prostorsmsru{
	...
	************************************************/
	public function Auth($module_id){
		$login = Option::get($module_id, 'CSR_PROSTORSMSRU_LOGIN');
		$passw = Option::get($module_id, 'CSR_PROSTORSMSRU_PASSWORD');
		
		return new prostorSMSrest($login, $passw);
	}
	...
	************************************************/
	public function sendSMS($module_id, $arFields){
        ...
        $arSend['sender'] = Option::get($module_id, 'CSR_PROSTORSMSRU_SENDERS', false);
	...
	************************************************/
	public function getSenders($tabControl, $module_id){
		...
		if($reply){
			foreach($reply as $s){
				$scur = explode(';', $s);
				$arSenders[$scur[0]] = $scur[0];
			}	
		}
		
		if (sizeof($arSenders) > 0) {
			$tabControl->AddDropDownField("CSR_PROSTORSMSRU_SENDERS", GetMessage("CSR_SENDERS"), false, $arSenders, Option::get($module_id, 'CSR_PROSTORSMSRU_SENDERS'));
		}
		else{
			$tabControl->AddViewField("CSR_PROSTORSMSRU_SENDERS", "", GetMessage("CSR_SENDERS_NOT_FOUND"));
		}
	}
	...
	************************************************/
	public function getForm($tabControl, $module_id){
		$login = Option::get($module_id, 'CSR_PROSTORSMSRU_LOGIN');
		$password = Option::get($module_id, 'CSR_PROSTORSMSRU_PASSWORD');
		
		$tabControl->AddEditField("CSR_PROSTORSMSRU_LOGIN", GetMessage('CSR_LOGIN'), false, array("size"=>30, "maxlength"=>255), $login);
		$tabControl->AddEditField("CSR_PROSTORSMSRU_PASSWORD", GetMessage('CSR_PASS'), false, array("size"=>30, "maxlength"=>255, "id"=>"password-type"), $password);
		...
		if(!empty($login) && !empty($password)){
			...
			if(is_array($balance)){
				$tabControl->AddViewField("CSR_PROSTORSMSRU_BALANCE", GetMessage("CSR_BALANCE"), $balance[1].' '.$balance[0]);	
	...
	************************************************/
	public function getToken($module_id){
		return Option::get($module_id, 'CSR_PROSTORSMSRU_TOKEN');
	}
